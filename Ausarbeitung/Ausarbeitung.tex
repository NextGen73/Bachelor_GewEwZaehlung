...
% irgendwo muss noch gezeigt werden, dass alle Eigenwerte auf der reellen Achse liegen, sonst könnte man sich fragen, ob die Gleichung fehlerhaft wird, wenn man über einen Kreis in IC integriert
      \section{Komplexe Analysis und der Residuensatz}
            ...
            \begin{theorem}(Theorem 9.2 (Residue theorem)\cite[S. 141]{complexAnalysis})
                  ...
                  Sei $f$ analytisch auf $\Omega$, mit Ausnahme der isolierten Singularitäten $a_1,...,a_n$. Wenn für einen Kreis $\gamma\subseteq\C$  $\gamma \sim 0$ und $a_j\notin \gamma,\ j=1,...,n$ gilt, dann
                  $$\int_\gamma f(z)\ dz = \frac{1}{2\pi i}\sum_{k=1}^{n} n(\gamma, a_k)\Res_{a_k}f$$
            \end{theorem}
...
% nicht alle sind fett markiert
                  \item $\mathbf{analytisch in z:}$ ist eine Eigenschaft von komplexen Funktionen, die besagt, dass man die Funktion als eine Potenzreihe um $z$ entwickeln kann.
...
% man müsste die Funktion referenzieren
                        ...
                        Offenbar sind $p$ und $q$ Polynome und $\{z: q(z)\ne 0\} = \C\setminus\{\lambda_i, i=1,...,n\}$
                        Daher ist $L$ analytisch auf $\C$ bis auf $\lambda_1,...,\lambda_n$
                  \item $\mathbf{b\text{ isolierte Singularität:}}$
                        ...
                        Offenbar ist $L(\lambda_i)$ nicht definiert für $i=1,...,n$ und da $L$ nur eine endliche Anzahl an Singularitäten besitzt, gilt:

                        \begin{equation}
                              \label{hilfe: complexAnalysis_isolierteSingularitäten}
                              f \text{ analytisch für } 0<|z-\lambda_i|<\widetilde{\epsilon},\quad i=1,...,n
                        \end{equation}
                        mit
                        $$\widetilde{\epsilon}:= \frac{1}{2}\,\min_{i,j\in\{1,...,n\}: \lambda_i\ne \lambda_j} |\lambda_i-\lambda_j|$$
...
% aus den Fingern gesaugt
                  ...
                  \item $n(\gamma, a)$:
                        ...
                        Man erkennt, dass mit $\gamma = C$\footnote{$C$ aus (\ref{hilfe: complexAnalysis_IntegralEinsDurchX}) gemeint}  und (\ref{hilfe: complexAnalysis_IntegralEinsDurchX}) gilt:
                        \begin{equation}
      ...
      % ist klar was mit Inneren eines Pfadintegrals gemeint ist?
                              \end{cases}
                        \end{equation}
                        Hierbei benötigt man das $C$ aus (\ref{hilfe: complexAnalysis_IntegralEinsDurchX}), damit der Kreis positiv orientiert ist und jeder Punkt des Kreises nur einmal durchlaufen wird.
...
% Zitat überprüfen/ übersetzen
      ...
      \section{Anwendung des Residuensatzes}
            ...
            $$J(s) = \int_\gamma \sum_{j=1}^{n} g(z)\frac{1}{z-\lambda_j}\ dz$$
