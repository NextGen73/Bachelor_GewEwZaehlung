...
% es fehlt noch: Einführung s und Erklärung, warum nun lambda statt w^2
      ...
      $$(K-\w^2 M)x = 0$$
      Gesucht ist eine Familie $(\w_j)_j$, die diese Gleichung für ein $x_j \ne 0$ löst.
      ...
      man will nun keine Eigenwerte mehr in dem Intervall $[\w_a^2,\w^2_b] =: [\lambda_a, \lambda_b]$
      ...
      Die Funktion ist nicht stetig differenzierbar und entscheidender: sie ist eine Treppenfunktion, damit sind alle Ableitungen gleich 0, also unbrauchbar für Minimierungsalogrithmen.

      Diese Probleme kann man abfedern, indem man die Funktion auf dem Intervall $[\lambda_a, \lambda_b]$ gewichtet.

      Nutze dazu eine Gewichtungsfunktion, die auf dem ganzen Intervall positiv ist und außerdem konkav, damit die Eigenwerte, die sich in dem Intervall befinden \zitat{herausgedrückt} werden.

      Nach \cite[S. 3]{hauptteilTkachuk} empfielt sich die Funktion
      $$g(z) = -(z-((1+\alpha)\lambda_a -\alpha\lambda_b))(z-((1+\alpha)\lambda_b-\alpha\lambda_a))$$
      wobei $\alpha$ der Inflations-Parameter (vgl. \cite[S. 3]{hauptteilTkachuk}) genannt wird.
